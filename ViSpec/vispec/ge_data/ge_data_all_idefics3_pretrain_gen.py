...
parser.add_argument(
    "--datapath",
    type=str,
    default="/data/llava_datasets/data/LLaVA-Pretrain",
    help="Root of LLaVA-Pretrain (blip_laion_cc_sbu_558k.json + images/).",
)
...
def build_dataset_rank(processor, path):
    ...
    def preprocess_function(examples):
        ...
        outputs = {
            "image_files": [resolve_image_path(path, examples["image"])],
            "text": prompt_input,
        }
        return outputs
...
ds = build_dataset_rank(processor, args.datapath)
print(ds)
...
@torch.no_grad()
def ge(data: Dict):
    images = [Image.open(f).convert("RGB") for f in data["image_files"]]
    ...
    image_mask = (outs_big.sequences == bigmodel.config.image_token_id)[..., :-1]
