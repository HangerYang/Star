                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Idefics3 / SmolVLM: text decoder nested under model.text_model
                            tensor = m.model.text_model.embed_tokens.weight.float()
                    del m
            except:
        ...
        if image_mask is not None and past_key_values is None:
            ...
            for b in range(bsz):
                ...
                for idx in range(num_ids):
                    ...
                    cur_txt = self.fc(torch.cat((txt_emd, hidden), dim=-1))
                    h_s.append(cur_txt)
                    ...
                    # NOTE: use the *current* segment's shapes (cur_txt / cur_img);
                    # image tokens may form multiple runs (e.g. Idefics3/SmolVLM tiles),
                    # so referencing the first segment (h_s[0]/h_s[1]) is incorrect.
                    t_m += [
                        eye_m[img_id_start : img_id_start + cur_txt.shape[0], :],
                        eye_m[img_id_end - cur_img.shape[0] : img_id_end, :],
                    ]
