...
import gc
try:  # plotting is optional and only used by the standalone __main__ demo
...
class LayerContributionAnalyzer:
    ...
    def compute_expected_accept_lengths(self) -> torch.Tensor:
        ...
        head_device = self.model.lm_head.weight.device
        final_layer_out = final_layer_out[:, -num_positions:, :].to(head_device)
        ...
        for layer_idx in range(self.num_layers):
            ...
            layer_out = layer_out[:, -num_positions:, :].to(head_device)  # (batch, num_positions, hidden_size)
