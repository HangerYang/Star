...
class LlamaRotaryEmbedding_L31(nn.Module):
    def __init__(
        ...
        if config is None:
            ...
            # Resolve rope_type across transformers versions:
            # - transformers >=5.0 stores rope config in `config.rope_parameters`
            # - older versions used `config.rope_scaling`
        ...
        self.config = config
        # transformers >=5.0 removed "default" from ROPE_INIT_FUNCTIONS and moved it
        # to a dedicated method; fall back to a local implementation for that case.
    ...
    def _compute_default_rope_parameters(config, device=None, seq_len=None, **kwargs):
        rope_params = getattr(config, "rope_parameters", None)
        if rope_params is not None and rope_params.get("rope_theta") is not None:
