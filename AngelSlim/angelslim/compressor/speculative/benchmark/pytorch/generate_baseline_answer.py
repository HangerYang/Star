...
def initialize_model(config: EvaluationConfig):
    """Initialize the regular Hugging Face baseline model."""
    ...
    for i in range(num_choices):
        ...
        for qs in question["turns"]:
            result = process_conversation_turn(
                model, tokenizer, conv, qs, temperature, max_new_token, top_p, top_k
            )
...
def get_model_answers(
    ...
    tokenizer = AutoTokenizer.from_pretrained(config.base_model_path, use_fast=False)
